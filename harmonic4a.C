...
main()
{
    double x, v, t,h ;
    k = 1; x = 1; v = 0;
    int i,nsteps, norbits;
    cerr << "Enter steps,  orbits:";
    cin >> nsteps >> norbits;
    h = 2*M_PI/nsteps;
    cout.precision(16);
    cerr  << " x= " << x << " v= " << v << " h = " <<  h <<"\n";
	...
	double errx = fabs(x-cos(phase));
	double errv = fabs(v+sin(phase)) ;
	if (emax < errx) emax = errx;
	if (emax < errv) emax = errv;
    }
    cout << "norb= " << norbits << " steps= " << nsteps <<
	" type=" << integrator_type << " emax= " << emax << "\n";
}
